...
form_class = uic.loadUiType(r"C:\Users\crebi\Documents\GitHub\PyQt5\Calculator\07_Calc_Sign\Calc.ui")[0]
...
class WindowClass(QMainWindow, form_class):
    ...
    def SignDisplay(self, inval):
        self.inVal = inval
        if self.operate == False:
            if self.inVal == '-':
                self.lineEdit_result.setText(self.inVal + self.lineEdit_result.text())
            else:
                self.lineEdit_result.setText(self.lineEdit_result.text().strip('-'))
    ...
    def InputButton_point(self):   
        if self.point == False:
            self.NumberDisplay('.')
            self.point = True

    def InputButton_sign(self):
        if self.sign == False:
            self.SignDisplay('-')
            self.sign = True
        else:
